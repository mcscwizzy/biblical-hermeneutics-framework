    ...
    *,
    ...
    with connect(path) as connection:
        ...
        if book is None and chapter is None:
            rows = connection.execute("SELECT * FROM saved_map_studies ORDER BY created_at DESC").fetchall()
        else:
            canonical, chapter_number = reference_filter(book, chapter)
            rows = connection.execute(
                "SELECT * FROM saved_map_studies WHERE book = ? AND chapter = ? ORDER BY created_at DESC",
...
def get_saved_map_study(study_id: str, path: str | Path = DEFAULT_DB_PATH, *, ensure_schema: EnsureSchema) -> dict[str, Any]:
    with connect(path) as connection:
        ensure_schema(connection)
        row = connection.execute("SELECT * FROM saved_map_studies WHERE id = ?", (study_id,)).fetchone()
    if row is None:
    ...
    with connect(path) as connection:
        ...
        connection.execute(
            """INSERT INTO saved_map_studies (
                ...
                archaeology_id, manuscript_id, selected_layers, map_view_state,
                generated_summary, user_notes, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                study_id, study["book"], study["chapter"], study["start_verse"], study["end_verse"],
                study["passage_reference"], study["selected_place_id"], study["selected_route_id"],
                study["selected_layer_id"], study["selected_archaeology_id"], study["selected_manuscript_id"],
                json.dumps(study["selected_layers"]), json.dumps(study["map_view_state"]),
                study["generated_summary"], study["user_notes"], now, now,
            ),
        )
    return {**study, "id": study_id, "created_at": now, "updated_at": now}


def delete_saved_map_study(study_id: str, path: str | Path = DEFAULT_DB_PATH, *, ensure_schema: EnsureSchema) -> bool:
    with connect(path) as connection:
        ensure_schema(connection)
        cursor = connection.execute("DELETE FROM saved_map_studies WHERE id = ?", (study_id,))
...
def saved_map_study_from_row(row: Any) -> dict[str, Any]:
    return {
        ...
        "selected_layers": [str(value) for value in _json_list(row["selected_layers"]) if str(value).strip()],
