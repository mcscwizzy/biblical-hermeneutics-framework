(() => {
  ...
  const syncThemeButton = () => { const button = $("[data-archaeology-theme]"); const isDark = document.documentElement.dataset.theme === "dark"; if (button) { button.textContent = isDark ? "Light mode" : "Dark mode"; button.setAttribute("aria-pressed", String(isDark)); } };
  document.addEventListener("DOMContentLoaded", () => { const form = $("[data-archaeology-search]"); form?.addEventListener("submit", (event) => { event.preventDefault(); load().catch((error) => { $("[data-archaeology-summary]").textContent = error.message; }); }); $("[data-archaeology-clear]")?.addEventListener("click", () => { form.reset(); load(); }); $("[data-archaeology-close]")?.addEventListener("click", () => $("[data-archaeology-detail]")?.close()); $("[data-archaeology-theme]")?.addEventListener("click", () => { const theme = document.documentElement.dataset.theme === "dark" ? "light" : "dark"; document.documentElement.dataset.theme = theme; try { localStorage.setItem("bhf-theme", theme); } catch (_error) {} syncThemeButton(); }); syncThemeButton(); load().catch((error) => { $("[data-archaeology-results]").textContent = error.message; }); });
})();
