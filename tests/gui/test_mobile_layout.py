...
def test_mobile_bottom_nav_switches_between_bible_and_ask(driver, wait, base_url):
    _set_mobile_viewport(driver)
    HomePage(driver, wait, base_url).open().wait_loaded()

    page = WorkspacePage(driver, wait, base_url)
    assert page.find('[data-testid="mobile-nav-bible"]').is_displayed()

    page.open_mobile_section("ask")
    wait.until(lambda _driver: _driver.execute_script("return document.body.dataset.mobileSection") == "ask")
    ...
    page.open_mobile_section("bible")
    wait.until(lambda _driver: _driver.execute_script("return document.body.dataset.mobileSection") == "bible")
